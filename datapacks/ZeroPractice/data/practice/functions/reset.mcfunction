...
# player
execute as @a[tag=!inSetup] in minecraft:the_end run tp @a 135 65 0 90 0
execute as @a[tag=!inSetup] if score onecycle flags matches 1 in minecraft:the_end run tp @a 135 65 0 90 -20
clear @a
effect give @a minecraft:instant_health 10 10 true
effect give @a[scores={death=1..}] minecraft:hunger 1 255
stopsound @a
execute as @a[tag=inSetup] in minecraft:the_end run function practice:setup/setup_start
